        ...
        <div class="col-md-12">
            <div class="card" style="padding-bottom: 15px;">
                <div class="card-header">
                    <h3 class="card-title">Ubah TravelTopia</h3>
                </div>
                <form method="POST" action="{{ url('app-admin/data/traveltopia/proses-ubah') }}"
                    ...
                    <div class="card-body">
                        ...
                        <div class="form-group">
                            ...
                            <input name="slug" class="form-control @error('slug') is-invalid @enderror" id="slug"
                                value="{{ old('slug') ?? $new->slug }}"type="text" required placeholder="Masukkan Slug">
                            @error('slug')
                                <div class="invalid-feedback">
                                    {{ $message }}
                                </div>
                        ...
                        <div class="form-group">
                            ...
                            <textarea id="content" name="content" placeholder="Masukkan Konten" rows="10" required>{{ old('content') ?? $new->content }}</textarea>
                            <div class="my-3"></div>
                        </div>
                        <div class="form-group">
                            <label for="content_en">Konten (Inggris)</label>
                            <textarea id="content_en" name="content_en" placeholder="Masukkan Konten" rows="10" required>{{ old('content_en') ?? $new->content_en }}</textarea>
                            <div class="my-3"></div>
                        </div>
                        <div class="form-group">
                            <label for="content_mandarin">Konten (Mandarin)</label>
                            <textarea id="content_mandarin" name="content_mandarin" placeholder="Masukkan Konten" rows="10" required>{{ old('content_mandarin') ?? $new->content_mandarin }}</textarea>
                            <div class="my-3"></div>
                        </div>
                        <div class="form-group">
                            <label for="image" class="form-label">Gambar</label>
                            <br>
                            <img id="addImage" src='{{ url("/assets/berita/$new->image") }}'
